...
Recorder1 的 MVP 主链路已经完成：菜单栏应用可启动，Gemini 已移除，Feishu CLI 上传链路已接入并通过端到端上传验证，黑色版菜单栏 UI 已完成。
...
- Feishu：
  - `lark-cli` 已安装并登录
  - 必要权限已验证
  - 端到端上传已成功拿到 `file_token`
  - 端到端生成妙记已成功拿到 `minute_url`
  - `vc +notes` 已成功返回转写文件
- 系统音频验收：
  - 签名验收构建不是 ad-hoc
